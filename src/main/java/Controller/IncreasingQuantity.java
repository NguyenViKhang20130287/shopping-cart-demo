...
@WebServlet(name = "IncreasingQuantity", value = "/IncreasingQuantity")
public class IncreasingQuantity extends HttpServlet {
    ...
    private void increasing(List<product> list, int pid) {
        for (product item : list) {
            if (item.getId() == pid) {
                if (item.getQuantityInCart() < new productDAO().getQuantityWithID(pid)) {
